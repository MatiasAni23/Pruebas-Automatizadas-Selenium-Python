...
class Pruebas_retail(unittest.TestCase):
    ...
    def grabar_multiples_ventanas(cls):
        ...
        for i, title in enumerate(windows_titles):
            ...
            record_thread = threading.Thread(target=cls.iniciar_grabacion, args=(i, title, output_file)) #Iniciar FFmpeg en un hilo separado
    ...
    def iniciar_grabacion(self,ventana_title):
        window = gw.getWindowsWithTitle(ventana_title)
        if not window:
            print(f"No se encontró la ventana {ventana_title}")
            return None

        hwnd = window[0]._hWnd
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        output_file = os.path.join(self.output_dir, f"grabacion_{ventana_title}_{timestamp}.mp4")
    ...
    def detener_grabacion(self):
        ...
        if hasattr(self, 'ffmpeg_process') and self.ffmpeg_process:
            try:
                # Enviar señal SIGINT para detener FFmpeg de forma ordenada
                self.ffmpeg_process.send_signal(subprocess.signal.CTRL_C_EVENT)
                self.ffmpeg_process.wait()  # Esperar a que el proceso termine
                print("Grabación detenida.")
            except Exception as e:
                print(f"Error al detener FFmpeg: {e}")
            finally:
                self.ffmpeg_process = None  # Limpiar la referencia al proceso
    ...
    def test_01_carga_pagina_falabella(self):
        ...
        self.iniciar_grabacion("Falabella")
        print("Test_01")
        print("Falabella")
        print("-" * 50)
    ...
    def test_02_boton_login(self):
        ...
        try:
            ...
            print("Clic realizado en el botón de login dentro del menú")
        except Exception as e:
            print(f"Error al buscar o hacer clic en el botón interno de inicio de sesión - Falabella: {e}")
            return
